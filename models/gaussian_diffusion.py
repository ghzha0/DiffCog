...
class GaussianDiffusion(nn.Module):
    def __init__(self, mean_type, noise_schedule, noise_scale, noise_min, noise_max, \
        ...
        self.Lt_history = th.zeros(steps, history_num_per_term, dtype=th.float32).to(device)
    ...
    def training_losses(self, model, x_start, reweight=False):
        ...
        # 知识结构constraint
        # delta_t = th.ones_like(ts).to(device)
        # delta_output = model(x_t, delta_t)
        # delta = delta_output - x_t
        # terms['constraint_loss'] = constraint_model(delta)
        ...
        # update Lt_history & Lt_count (Trick)
        temp_l = loss.detach()
        ts_1_i = th.where(self.Lt_count[ts] == self.history_num_per_term)
        ts_1 = ts[ts_1_i]
        ts_2_i = th.where(self.Lt_count[ts] < self.history_num_per_term)
        ts_2 = ts[ts_2_i]

        if len(ts_1) > 0:
            self.Lt_history[ts_1] = th.roll(self.Lt_history[ts_1], shifts=-1, dims=1)
            self.Lt_history[ts_1, -1] = temp_l[ts_1_i].mean()
        if len(ts_2) > 0:
            self.Lt_history[ts_2, self.Lt_count[ts_2]] = temp_l[ts_2_i].mean()
            self.Lt_count[ts_2] += 1
        ...
        #         self.Lt_history[t] = th.roll(self.Lt_history[t], shifts=-1, dims=0)
...
def masked_mean_flat(tensor, mask):
    tensor = tensor * mask
    return tensor.sum(dim=list(range(1, len(tensor.shape)))) / mask.sum(dim=list(range(1, len(tensor.shape))))
